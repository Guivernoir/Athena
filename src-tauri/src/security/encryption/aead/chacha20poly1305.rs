//! **ChaCha20-Poly1305** AEAD — zero-copy, ≤ 5 KB, zeroize-ready
...
use chacha20poly1305::{aead::AeadInPlace, ChaCha20Poly1305, KeyInit, Nonce as C20Nonce};
use zeroize::Zeroize;
...
pub fn encrypt(key: &Key, nonce: &Nonce, plaintext: &[u8]) -> Result<Vec<u8>, EncryptionError> {
    ...
    let n = C20Nonce::from_slice(nonce.as_bytes());
...
pub fn decrypt(key: &Key, nonce: &Nonce, ciphertext: &[u8]) -> Result<Vec<u8>, EncryptionError> {
    ...
    let result = cipher.decrypt(n, &mut out);
    if result.is_err() {
        secure_zero(&mut out);
        return Err(EncryptionError::MacMismatch);
    }
    Ok(out)
...
// ------------------------------------------------------------------
// 4. Key + Nonce generation helpers
// ------------------------------------------------------------------
#[inline(always)]
...
mod ffi {
    ...
    pub unsafe extern "C" fn chacha20poly1305_encrypt_raw(
        ...
        let key = Key::new(key_slice.try_into().unwrap());
        let nonce = Nonce::new(nonce_slice.try_into().unwrap());
