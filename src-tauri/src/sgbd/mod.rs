...
pub use tx::{TransactionManager, Transaction, TxOperation};
pub use types::{Key, Value, InputRecord, RecordMetadata, SGBDError, Result};
...
mod tests {
    ...
    async fn test_basic_operations() {
        ...
        let key = engine.store_input_record(test_input.to_string(), parsed_input).await.unwrap();
